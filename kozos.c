				 ...
				 int stacksize, int argc, char *argv[])
{
   ...
   */
  thp->context.cpsr = 0xc4; /* spsr_el1 */

  /* スレッドのスタート・アップ(thread_init())に渡す引数 */
  thp->context.x[0] = (uint32)thp;  /* x0 */

  thp->context.x[30] = (uint32)thread_exit; /* x30 return addr */

  thp->context.pc = (uint32)thread_init; /* elr_el1 */

  thp->context.sp = (uint32)sp;

  puts("generate thread, name : ");
  puts(name);
  puts("\n");
...
static void thread_intr(softvec_type_t type, unsigned long sp)
{
  ...
  memcpy(&current->context, sp, sizeof(kz_context));
  /*
   * 割込みごとの処理を実行する．
